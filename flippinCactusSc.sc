...
__on_player_right_clicks_block(player, item_tuple, hand, block, face, hitvec) ->(
    ...
    schedule(5,'_flip_block',block,properties,player~'sneaking');//so u can reset repeater delay
    //if(block=='hopper',set(block,'air'));
    //run(str('player %s sneak',player));
);
